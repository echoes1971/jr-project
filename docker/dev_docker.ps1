...
if($MYSQL_EXISTS) {
    ...
    if($MYSQL_ENGINE -eq "mariadb") {
        docker exec -it $MYSQL_APP mariadb -uroot --password=${MYSQL_PASSWORD} -e "create database if not exists $MYSQL_DB;"
    } else {
    ...
    while($retVal -ne 0) {
        ...
        if($MYSQL_ENGINE -eq "mariadb") {
            docker exec -i $MYSQL_APP mariadb -uroot --password=${MYSQL_PASSWORD} -e "source /tmp/sql/db_rproject.sql"
        } else {
            docker exec -i $MYSQL_APP mysql -uroot --password=${MYSQL_PASSWORD} -e "source /tmp/sql/db_rproject.sql"
        }
        $retVal=$LASTEXITCODE
...
Write-Output "Access mysql with: docker exec -it $MYSQL_APP mariadb -p$MYSQL_PASSWORD $MYSQL_DB"
